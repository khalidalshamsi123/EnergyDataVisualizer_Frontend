    ...
    <div class="graph-container rounded bg-white shadow-md mb-8">
      <div v-for="option in selectedOptions">
        <template v-if="option === 'Pie Chart'">
          <PieChart v-if="pieChartData" :key="pieChartData.title" :title="pieChartData.title" :data="pieChartData.data"
            :as-percentage="pieChartData.asPercentage" />
        </template>
        <template v-if="option === 'Bar Chart'">
          <BarChart v-if="barChartData" :key="barChartData.title" :title="barChartData.title" :data="barChartData.data"
            :max="barChartData.max" :legend="barChartData.legend" />
        </template>
      </div>
    </div>
          ...
          <div class="flex p-1 pt-0 mr-auto" v-if="chartTypesDowndownOpen">
            <div class="heat-cold-options bg-white rounded p-4 ml-2 h-[8.5rem]">
              <div class="heat-container">
                <input type="radio" id="heat" v-model="selectedHeatDwelling" value="heating_type" class="mr-2" />
                <label for="heat" class="cursor-pointer">Heating Type</label>
              </div>
              <div class="heat-container">
                <input type="radio" id="dwelling" v-model="selectedHeatDwelling" value="dwelling_type" class="mr-2" />
            ...
            <div class="before-after-options bg-white rounded p-3 h-[8.5rem]">
              <div class="option-container">
                <input type="radio" id="average" v-model="selectedAverageSum" value="average" class="mr-2" />
            ...
            <div class="top-full w-96 bg-white rounded shadow p-4 h-[8.5rem] ml-2">
              <div class="grid grid-cols-3 gap-4 justify-end">
                <div v-for="option in ['Pie Chart', 'Bar Chart', 'Line Graph']" :key="option" class="option-container">
        ...
        <div class="relative">
          ...
          <div class="p-1 pt-0 mr-auto" v-if="downloadDropbarOpen">
            <div class="download-snippet-options bg-white rounded p-3 h-[8.5rem]">
              <div class="option-container">
                <input type="radio" id="snippet" v-model="SelectDownloadSnippet" value="Snippet" class="mr-2" />
              ...
              <div class="option-container">
                <input type="radio" id="download" v-model="SelectDownloadSnippet" value="Download" class="mr-2" />
...
watch([selectedHeatDwelling, selectedOptions, selectedAverageSum], async () => {
  ...
  const { data: jsonData } = await useFetch(`${config.public.baseUrl}/api/Energy_efficiency_improvements_costs_LA`, {
    method: "post",
    body: data_required,
    transform: (response) => {
  ...
  if (selectedAverageSum.value === "average") {
    ...
    title = `Breakdown of average energy efficiency improvement costs per ${selectedHeatDwelling.value === "heating_type" ? "heating technology" : "dwelling type"} ${asPercentage ? "(%)" : "(£)"}`;
  } else if (selectedAverageSum.value === "sum") {
    chartData = jsonData.value[`${selectedHeatDwelling.value}`]["sum"]
    title = `Breakdown of overall energy efficiency improvement costs per ${selectedHeatDwelling.value === "heating_type" ? "heating technology" : "dwelling type"} ${asPercentage ? "(%)" : "(£Million)"}`;
  }
